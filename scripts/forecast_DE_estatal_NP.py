#!/usr/bin/env python3
"""
forecast_DE_estatal_NP.py — Contratos / P. Contratadas estatal — NeuralProphet
"""
import argparse
import json
import sys
...
from config import NP_DE_ESTATAL_PARAMS
...
def main():
    ...
    try:
        ...
        train_months = NP_DE_ESTATAL_PARAMS['cv']['train_months']
        val_months   = NP_DE_ESTATAL_PARAMS['cv']['val_months']
        step_months  = NP_DE_ESTATAL_PARAMS['cv']['step_months']
        ...
        # n_lags=0: Contratos no usa autoregresión — NP predice desde tendencia y estacionalidad
        NLAGS     = NP_DE_ESTATAL_PARAMS['nlags']
        FREQ_DATA = 'MS'
        ...
        #   (especialmente relevante para Contratos, muy sensibles a cambios normativos y económicos)
        ...
        for ci, params in enumerate(grid):
            ...
            while start + train_months + val_months <= len(df_np):
                ...
                # 3.3. Sin lags, NP predice directamente sobre las fechas de val_df
                #      usando solo tendencia y estacionalidad — no necesita valores reales pasados
                preds     = m_fold.predict(val_df)
        ...
        # make_future_dataframe genera las fechas futuras como DataFrame vacío (y=None).
        # Con n_lags=0 no se necesita predicción recursiva: NP extrapola directamente
        # la tendencia y estacionalidad aprendidas.
        # n_historic_predictions=False: devuelve solo el horizonte futuro, sin el histórico.
        future   = m_final.make_future_dataframe(df_np, periods=HORIZONTE_MESES, n_historic_predictions=False)
        ...
        pronostico = [
            {'fecha': row['ds'].strftime('%Y-%m'), 'valor': round(float(row['yhat1']))}
