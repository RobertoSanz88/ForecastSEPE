#!/usr/bin/env python3
"""
forecast_ABC_atributo_NP.py — Parados/Afiliados/Demandantes por AtributoX — NeuralProphet
"""
import argparse
import json
import sys
...
from config import NP_ABC_ATRIBUTO_PARAMS
...
def main():
    ...
    try:
        ...
        NLAGS     = NP_ABC_ATRIBUTO_PARAMS['nlags']
        val_months = NP_ABC_ATRIBUTO_PARAMS['val_months']
        FREQ_DATA = 'MS'
        ...
        # n_lags=2: NeuralProphet usa los 2 meses anteriores como input — requiere predicción recursiva
        ...
        # val_months=12 es suficiente para discriminar entre combinaciones.
        ...
        # valid incluye NLAGS filas extra al inicio para que la predicción recursiva tenga contexto
        train_data = df_new[:-val_months]
        valid_data = df_new[-val_months - NLAGS:]
        ...
        def recursive_predict_neuralprophet(model, train_data, periods=12):
            """
            Predicción recursiva mes a mes con NeuralProphet cuando n_lags > 0.
            """
            predictions = []
            future = model.make_future_dataframe(df=train_data, periods=1)
            ...
            for _ in range(periods):
                ...
                forecast.loc[len(forecast) - 1, 'y'] = prediction
                future = model.make_future_dataframe(df=forecast[['ds', 'y']], periods=1)

            return predictions

        # 3. OPTIMIZACIÓN DE HIPERPARÁMETROS (Grid Search manual, validación en el último año)
        # ==================================================================================
        ...
        params_grid = NP_ABC_ATRIBUTO_PARAMS['grid']
        ...
        for gi, (grupo, grupo_df) in enumerate(train_dataframes.items()):
            ...
            for i, p in enumerate(grid, start=1):
                ...
                # Predicción recursiva sobre los meses de validación (sin los NLAGS iniciales)
                valid_forecast = recursive_predict_neuralprophet(model, grupo_df, periods=val_months)
                mape = mean_absolute_percentage_error(
                    valid_dataframes[grupo]['y'][NLAGS:], valid_forecast
                ) * 100
            ...
            forecasts_valid[grupo] = recursive_predict_neuralprophet(model, grupo_df, periods=val_months)
        ...
        for grupo in train_dataframes:
            mape_train_values[grupo] = mean_absolute_percentage_error(
                train_dataframes[grupo]['y'][NLAGS:],
                forecasts_train[grupo][NLAGS:]['yhat1']
            ) * 100

        for grupo in valid_dataframes:
            mape_valid_values[grupo] = mean_absolute_percentage_error(
                valid_dataframes[grupo]['y'][NLAGS:],
                forecasts_valid[grupo]
            ) * 100
            mae_valid_values[grupo] = mean_absolute_error(
                valid_dataframes[grupo]['y'][NLAGS:],
                forecasts_valid[grupo]
            )
        ...
        # 7. ENTRENAMIENTO FINAL Y PRONÓSTICO
        # ==================================================================================
        ...
        for i, (grupo, grupo_df) in enumerate(trainvalid_dataframes.items(), start=1):
            ...
            forecasts_results_np[grupo] = recursive_predict_neuralprophet(
                model, grupo_df, periods=HORIZONTE_MESES
            )
        ...
        for grupo in grupos:
            ...
            pronostico = [
                ...
                for d, v in zip(forecasts_dates, forecasts_results_np[grupo])
            ]
            series_out[grupo] = {
                'historico':  historico,
